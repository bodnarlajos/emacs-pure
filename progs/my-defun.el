...
(defun my/menu-base ()
	...
	(let ((replaceString "Replace string")
				...
				(loadDesktop "Load desktop")
				(projectCompile "Project compile"))
		(let ((ido-list (list recentfFiles runCommand revertBuffer rg rgCurrent backTo development openNotes magit vcdir longLines replaceString jumpTo flymakeList projectFindFile elScrn elScrnNext projectCompile findnamedired loadDesktop)))
			(let ((res (selectrum-completing-read "Action: " ido-list)))
				(cond				
				 ...
				 ((string-equal res loadDesktop) (call-interactively 'desktop-read))
